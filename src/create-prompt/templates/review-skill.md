...
## Bug-Dense Cluster Persistence

After finding one high-confidence issue in a changed cluster, do one additional
pass over that same cluster for independent failure mechanisms that share the
changed inputs, branches, state writes, side effects, or outputs. Keep the same
Reporting Gate: do not report extra issues unless you can name the changed-code
trigger, the observable wrong behavior, and why it is a separate root cause.
