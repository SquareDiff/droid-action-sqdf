...
## Confirmed-Issue Second Look

When you have already verified a high-confidence issue in a changed file
cluster, make one short second look over that same cluster before finalizing.
The goal is to avoid stopping early when a separate adjacent root cause is
already supported by the same changed code, while preserving the normal
reporting threshold.

Keep an additional candidate from this second look only when all are true:

- it is a different root cause from the first finding;
- it is triggered by changed code in the same cluster;
- it shares a concrete input, branch, state write, side effect, or output with
  the verified issue;
- it independently satisfies the Reporting Gate and validation rules below.

If the second look only finds variants of the same bug, prerequisites for the
first bug, style concerns, weaker P2s, or findings that need a different anchor
to be persuasive, do not report them.
